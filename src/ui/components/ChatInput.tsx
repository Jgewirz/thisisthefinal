...
import { starterChipLabels } from '../../app/agentStarters';
...
export function ChatInput({ agentId, onSend, disabled }: ChatInputProps) {
  ...
  return (
    <div className="shrink-0 pb-4 pb-safe" style={{ backgroundColor: 'var(--bg-primary)' }}>
      {/* Quick-action chips */}
      ...
      {pendingImage && (
        <div className="px-4 pb-3 flex items-center gap-3">
          ...
          <p className="text-xs leading-relaxed" style={{ color: 'var(--text-muted)' }}>
            {agentId === 'style' ? "Add a message or tap send — I'll analyze it!" : 'Image attached. Type a message and send.'}
          </p>
        </div>
      )}
      ...
      <div className="px-4">
        ...
        >
          ...
          <button
            ...
            onMouseLeave={(e) => (e.currentTarget.style.backgroundColor = 'transparent')}
          >
            {locationStatus === 'requesting' ? <Loader2 size={17} className="animate-spin" /> : <MapPin size={17} />}
          </button>
          ...
          >
            <Send size={15} style={{ color: canSend ? '#fff' : 'var(--text-muted)' }} />
          </button>
        </div>
      </div>
    </div>
