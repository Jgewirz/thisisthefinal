...
import { Loader2, Moon, AlertCircle, Check, ExternalLink } from 'lucide-react';
import { linkHatch } from '../../lib/api';

interface HatchLinkFormProps {
  onLinked: (email: string, deviceCount: number) => void;
  onCancel: () => void;
}

export function HatchLinkForm({ onLinked, onCancel }: HatchLinkFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const result = await linkHatch(email, password);
      if (result.success) {
        setDeviceCount(result.deviceCount || 0);
        setSuccess(true);
        setTimeout(() => onLinked(email, result.deviceCount || 0), 800);
      } else {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        >
          <Moon size={16} style={{ color: accentColor }} />
        </div>
        <div>
          <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>
            Link Your Hatch Account
          </p>
          <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
            One-time setup to control your Hatch device
          </p>
        </div>
      </div>
      ...
      <p className="text-xs" style={{ color: 'var(--text-secondary)' }}>
        Use your Hatch app login credentials.{' '}
        <a
          href="https://www.hatchwearables.com"
          target="_blank"
